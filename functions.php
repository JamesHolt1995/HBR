...
function theme_files()
{
 // wp_enqueue_script('googleMap', '//maps.googleapis.com/maps/api/js?key=AIzaSyDin3iGCdZ7RPomFLyb2yqFERhs55dmfTI', NULL, '1.0', true);
  ...
  wp_enqueue_style('custom-google-fonts', '//fonts.googleapis.com/css2?family=Work+Sans:ital,wght@0,400;0,600;0,700;1,400');
...
function theme_features()
{
  ...
  add_editor_style(
    array(
      'https://fonts.googleapis.com/css2?family=Work+Sans:ital,wght@0,400;0,600;0,700;1,400',
...
class Block
{
  ...
  function onInit()
  {
    ...
    register_block_type("blocks/{$this->name}", $args);
  }
}

new Block('banner', true, ['fallbackimage' => get_theme_file_uri( '/images/test.jpg' )]);
new Block('genericheading');
new Block('genericbutton');
...
function register_acf_blocks() {
    ...
    //test
    register_block_type( __DIR__ . '/blocks/acf/testimonial' );
    wp_register_script('testimonial_scripts', get_template_directory_uri() . '/build/testimonial.js', array(), _S_VERSION);
    
    //video banner
    register_block_type( __DIR__ . '/blocks/acf/video-banner' );
    wp_register_script('video_banner_scripts', get_template_directory_uri() . '/build/video-banner.js', array(), _S_VERSION);

    //header
    register_block_type( __DIR__ . '/blocks/acf/header' );
    wp_register_script('header_scripts', get_template_directory_uri() . '/build/header.js', array(), _S_VERSION);
}
 ...
 * CLS issues 
 */
add_action('wp_enqueue_scripts', 'acf_block_files');
add_action('admin_enqueue_scripts', 'acf_block_files');
